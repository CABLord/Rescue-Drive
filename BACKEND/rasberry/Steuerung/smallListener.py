...
class Listenersmall(threading.Thread):
    ...
    def run(self):
        ...
        while self.arduino.is_open:
            ...
            if (self.dataclass.steuerung_aut):
                ...
                if (self.arduino.in_waiting > 0):
                    ...
                    if (key == "DRIVE"):
                        ...
                        if (self.lastakku != value):
                            self.lastakku = value
                            send_json({"TIMESTAMP": datetime.now().strftime("%d/%m/%Y %H:%M:%S"),
                                       "AKKU": round(float(value))})
                    else:
            ...
            if (imkey in key):  # controll if message is from wished sensor
                ...
                if (key == "DRIVE"):
                    ...
                    if (self.lastakku != value):
                        self.lastakku = value
                        send_json(
                            {"TIMESTAMP": datetime.now().strftime("%d/%m/%Y %H:%M:%S"), "AKKU": round(float(value))})
                else:
    ...
    def process_color(self, color, mode):
        ...
        if (mode == "auto"):
            print(colors)
        if (redValue >= 180 and greenValue <= 180 and blueValue <= 180):
            if (mode == "auto"):
                self.arduino.write("stop\n".encode())
            color = "RED"
        ...
        elif (redValue <= 180 and greenValue <= 180 and blueValue >= 180):
            if (mode == "auto"):
                self.arduino.write("stop\n".encode())
            color = "BLUE"
    ...
    def control_distance(self, distance):
        ...
        if (distances[1] < 80):
            ...
            # self.arduino.write(("left" + "\n").encode())
            time.sleep(0.1)
        elif distances[1] > (SAFE_DISTANZ + 50):
